...
Point randPosSnake(World & w){ // Choose a random position for the stars or the insects or the Snake (I should change the name)
    Point res;
...
void writeHighScore(int intHighScore, bool haveImage){ // Write to a text file the highscore depending on if one played with the background or not
    if(haveImage){
...
void getHighScore(int & intHighScore, bool haveImage){ // Read the highscore from the aforementioned text files 
    if(haveImage){
...
void initWorld(World & w, bool haveImage){ // World structure initialized
    int x, y;
...
bool isGameOver(World w){ // Test to see if the player has lost (hit the borders or hit an obstacle)
...
bool notInSnake(World & w, Point pos){ // Funtion that tests whether or not a postion is inside the snake's body or not
...
bool notInStar(World w, Point pos){ // Test to know if a position is inside the star or not
    if(w.star.pos == pos){
        return false;
    }else{
        return true;
...
void updateWorld(World & w, Population & pop, bool & rainbow){ // Update of all the data that need regular updating (position, obstacles, has the snake eaten or not)
...
void drawWorld(World & w, Population & pop, bool haveImage, bool rainbow){ // Draw everything that needs to be drawn
    if(haveImage)
...
void isStarTimerFinished(World & w, bool & rainbow, bool & timerFinito){ // Test to know if after one eats a star, after 4 seconds (4*nulber of stars chained) the speed boost the colours and the combo are finished or not
...
bool starting(){ // Start of the game that asks if the individual would like to play with the background or not
    winClearEvent();
    color(0, 255, 0);
    fontSize(30);
...
bool ending(World & w, Population & pop , bool & rainbow, bool & speedSet, bool & timerFinito, bool & haveImage){ // End of the program that show the score and highscore and asks if one would like to play again
    if(!isGameOver(w))
        return false;
